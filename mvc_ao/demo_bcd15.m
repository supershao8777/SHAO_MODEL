% demo_bcd15.m  —  BCD-MVRL19 (doubly-stochastic C + simplex S/Z)
%
%  Objective: Σ||X-R(Ps+Q)Z||²+λ||C Ps Z-S||²+μ⟨D,C⟩+β||Q||²
%  Q≥0, Z/S col-simplex, C: Sinkhorn doubly stochastic
...
best_acc=0;best_nmi=0;best=struct('m',0,'l',0,'u',0,'b',0);
cnt=0;total=length(m_grid)*length(lam_grid)*length(mu_grid)*length(beta_grid);
for mi=1:length(m_grid)
    for li=1:length(lam_grid)
        for ui=1:length(mu_grid)
            for bi=1:length(beta_grid)
                cnt=cnt+1;try
                    t0=tic;[~,~,~,~,Sq,~,obj_q]=bcd_mvrl19(X,m_grid(mi),c,lam_grid(li),mu_grid(ui),beta_grid(bi),opts_q);
                    t1=toc(t0);res_q=myNMIACCwithmean(Sq',Y,c);
                    fprintf('%-4d %-6.2f %-6.2f %-6.2f %-9.4f %-9.4f %-9.2e %5.1fs [%d/%d]\n',...
                            m_grid(mi),lam_grid(li),mu_grid(ui),beta_grid(bi),res_q(1),res_q(2),obj_q(end),t1,cnt,total);
                    if res_q(1)>best_acc,best_acc=res_q(1);best_nmi=res_q(2);
                        best.m=m_grid(mi);best.l=lam_grid(li);best.u=mu_grid(ui);best.b=beta_grid(bi);end
                catch ME,fprintf('%-4d %-6.2f %-6.2f %-6.2f FAILED:%s [%d/%d]\n',...
                            m_grid(mi),lam_grid(li),mu_grid(ui),beta_grid(bi),ME.message,cnt,total);end
            end
        end
    end
end
if best.m==0,best.m=2*c;best.l=1;best.u=0.1;best.b=0.1;end
fprintf('\nBest: m=%d λ=%.2f μ=%.2f β=%.2f | ACC=%.4f NMI=%.4f\n',best.m,best.l,best.u,best.b,best_acc,best_nmi);

%% Refined
fprintf('\n=== Refined ===\n');opts.max_iter=100;opts.verbose=true;
tic;[~,~,~,~,S_b,~,obj_b]=bcd_mvrl19(X,best.m,c,best.l,best.u,best.b,opts);t_b=toc;
...
title(sprintf('BCD-MVRL19 m=%d λ=%.2f μ=%.2f β=%.2f ACC=%.4f',best.m,best.l,best.u,best.b,best_acc));grid on;
