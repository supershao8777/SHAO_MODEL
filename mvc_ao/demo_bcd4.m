% demo_bcd4.m  —  Demo for BCD-MVRL6 (Stiefel Ps + Sylvester Q)
%
%  Objective:
%    min Σ_v ||X^{(v)}-R^{(v)}(Ps+Q^{(v)})A^{(v)}||² + λ||(Ps+Q^{(v)})A^{(v)}-S||² + β||Ps^T Q^{(v)}||²
%
%  Constraints:
%    Ps^T Ps = I_c (Stiefel),  A,S col-stochastic,  R,Q unconstrained
%
%  Updates:
%    S  — simplex mean,  A  — closed-form + simplex
%    R  — pseudo-inverse,  Ps — GD + SVD Stiefel
%    Q  — Kronecker Sylvester  (exact closed-form)
...
for mi = 1:length(m_grid)
    for li = 1:length(lambda_grid)
        for bi = 1:length(beta_grid)
            ...
            try
                t0 = tic;
                [~, ~, ~, ~, Sq, obj_q] = bcd_mvrl6(X, m_grid(mi), c, ...
...
tic;
[R_b, Ps_b, Q_b, A_b, S_b, obj_b] = bcd_mvrl6(X, best_m, c, best_lambda, best_beta, opts);
t_b = toc;
...
title(sprintf('BCD-MVRL6 Best (m=%d,\\lambda=%.2f,\\beta=%.3f) ACC=%.4f', ...
