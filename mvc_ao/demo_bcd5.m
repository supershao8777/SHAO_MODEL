% demo_bcd5.m  —  Demo for BCD-MVRL7 (L21 row-sparse Q)
%
%  Objective:
%    min Σ_v ||X^{(v)}-R^{(v)}(Ps+Q^{(v)})A^{(v)}||² + λ||Ps A^{(v)}-S||² + β||Q^{(v)}||_{2,1}
%
%  Key features:
%    - Consensus term uses Ps only (not Ps+Q)
%    - Q regularized by L21 norm (row-sparsity / group lasso)
%    - Ps constrained to Stiefel manifold
%    - Q update via proximal gradient (group soft-thresholding)
%
%  Constraints: Ps^T Ps=I_c, A,S col-stochastic
...
for mi = 1:length(m_grid)
    for li = 1:length(lambda_grid)
        for bi = 1:length(beta_grid)
            ...
            try
                t0 = tic;
                [~, ~, ~, ~, Sq, obj_q] = bcd_mvrl7(X, m_grid(mi), c, ...
...
tic;
[R_b, Ps_b, Q_b, A_b, S_b, obj_b] = bcd_mvrl7(X, best_m, c, best_lambda, best_beta, opts);
t_b = toc;
...
title(sprintf('BCD-MVRL7 Best (m=%d,\\lambda=%.2f,\\beta=%.3f) ACC=%.4f', ...
