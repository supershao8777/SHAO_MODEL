...
%     end;
    maxlbl = max(incluster);
    if isempty(maxlbl) || maxlbl < 1
        continue;
    end
    inclunub = zeros(1, maxlbl);
    for lbl = 1:maxlbl
        inclunub(lbl) = sum(incluster == lbl);
    end
...
[nmi_val Entropy] = compute_nmi(Y, predY);
...
result = [ACC nmi_val Purity Fscore Precision Recall AR Entropy];
